# Switch2Trae 插件发布指南

本指南详细介绍如何将 Switch2Trae 插件发布到 JetBrains 插件市场。

## 📋 发布前检查清单

### ✅ 代码质量检查
- [ ] 所有功能正常工作
- [ ] 代码通过编译，无语法错误
- [ ] 已修复所有已知 Bug
- [ ] 代码符合 Kotlin 编码规范
- [ ] 添加了适当的注释和文档

### ✅ 插件配置检查
- [ ] `plugin.xml` 配置完整
- [ ] 版本号正确设置
- [ ] 插件描述清晰准确
- [ ] 兼容性设置正确
- [ ] 图标文件存在且符合规范

### ✅ 测试验证
- [ ] 在目标 IDE 版本中测试
- [ ] 快捷键功能正常
- [ ] 菜单项显示正确
- [ ] 设置页面工作正常
- [ ] 错误处理机制有效

## 🔧 第一步：准备开发者账户

### 1.1 创建 JetBrains 账户
1. 访问 [JetBrains 账户页面](https://account.jetbrains.com/)
2. 注册新账户或使用现有账户登录
3. 完善个人资料信息

### 1.2 申请开发者权限
1. 访问 [JetBrains Marketplace](https://plugins.jetbrains.com/)
2. 点击右上角 "Upload plugin"
3. 首次上传需要验证开发者身份
4. 填写开发者信息表单

## 🛠️ 第二步：完善插件配置

### 2.1 更新 plugin.xml
确保以下信息完整且准确：

```xml
<idea-plugin>
    <!-- 插件唯一标识符 -->
    <id>com.github.yan.switch2trae</id>
    
    <!-- 插件名称 -->
    <name>Switch2Trae</name>
    
    <!-- 版本号 -->
    <version>1.0.0</version>
    
    <!-- 开发者信息 -->
    <vendor email="yanwanglaiye@proton.me" url="https://github.com/zhongyuanyidiankeng">Yan</vendor>
    
    <!-- 插件描述 -->
    <description><![CDATA[
        详细的插件功能描述...
    ]]></description>
    
    <!-- 更新日志 -->
    <change-notes><![CDATA[
        v1.0.0:
        - 初始版本发布
        - 支持项目和文件切换
        - 智能光标位置同步
        - 多种命令行格式支持
    ]]></change-notes>
    
    <!-- 兼容性设置 -->
    <idea-version since-build="251" until-build="251.*"/>
    
    <!-- 依赖 -->
    <depends>com.intellij.modules.platform</depends>
</idea-plugin>
```

### 2.2 更新 build.gradle.kts
确保构建配置正确：

```kotlin
intellijPlatform {
    pluginConfiguration {
        // 版本信息
        version = "1.0.0"
        
        // IDE 版本兼容性
        ideaVersion {
            sinceBuild = "251"
            untilBuild = "251.*"
        }
        
        // 更新日志
        changeNotes = """
            <h3>v1.0.0 - 初始版本</h3>
            <ul>
                <li>✨ 支持 JetBrains IDE 与 Trae IDE 之间的无缝切换</li>
                <li>🎯 智能光标位置同步</li>
                <li>⌨️ 快捷键支持 (Alt+Shift+P, Alt+Shift+O)</li>
                <li>🔧 多种命令行格式支持</li>
                <li>🛠️ 灵活的配置选项</li>
            </ul>
        """.trimIndent()
    }
    
    publishing {
        // 发布配置
        token = providers.environmentVariable("PUBLISH_TOKEN")
        channels = listOf("default")
    }
}
```

### 2.3 准备插件图标
- 创建 `pluginIcon.svg` 文件（40x40 像素）
- 确保图标清晰、简洁、符合 JetBrains 设计规范
- 放置在 `src/main/resources/META-INF/` 目录下

## 🏗️ 第三步：构建插件

### 3.1 本地构建测试
```bash
# 清理项目
./gradlew clean

# 编译代码
./gradlew compileKotlin

# 运行测试
./gradlew test

# 构建插件
./gradlew buildPlugin
```

### 3.2 验证插件包
```bash
# 插件包位置
ls build/distributions/

# 应该看到类似文件：
# Switch2Trae-1.0.0.zip
```

### 3.3 本地安装测试
1. 打开 IDE → Settings → Plugins
2. 点击齿轮图标 → Install Plugin from Disk
3. 选择构建的 `.zip` 文件
4. 重启 IDE 并测试所有功能

## 📤 第四步：发布到市场

### 4.1 手动上传方式

#### 准备发布材料
1. **插件包**：`build/distributions/Switch2Trae-1.0.0.zip`
2. **插件描述**：准备详细的功能说明
3. **截图**：准备 2-5 张功能演示截图
4. **标签**：选择合适的分类标签

#### 上传步骤
1. 访问 [JetBrains Marketplace](https://plugins.jetbrains.com/)
2. 登录开发者账户
3. 点击 "Upload plugin"
4. 填写插件信息：
   - **Plugin name**: Switch2Trae
   - **Category**: Tools Integration
   - **Tags**: ide-integration, productivity, tools
   - **Description**: 使用 README 中的详细描述
   - **License**: MIT
5. 上传插件包文件
6. 添加截图和演示图片
7. 提交审核

### 4.2 自动化发布方式

#### 配置 GitHub Actions
创建 `.github/workflows/publish.yml`：

```yaml
name: Publish Plugin

on:
  release:
    types: [published]

jobs:
  publish:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      
      - name: Setup Java
        uses: actions/setup-java@v4
        with:
          distribution: 'temurin'
          java-version: '21'
          
      - name: Setup Gradle
        uses: gradle/gradle-build-action@v2
        
      - name: Publish Plugin
        env:
          PUBLISH_TOKEN: ${{ secrets.PUBLISH_TOKEN }}
        run: ./gradlew publishPlugin
```

#### 配置发布令牌
1. 在 JetBrains Marketplace 生成 API Token
2. 在 GitHub 仓库设置中添加 Secret：
   - Name: `PUBLISH_TOKEN`
   - Value: 你的 API Token

#### 使用 Gradle 发布
```bash
# 设置环境变量
export PUBLISH_TOKEN="your-api-token"

# 发布插件
./gradlew publishPlugin
```

## 🔍 第五步：审核流程

### 5.1 审核时间
- **首次提交**：通常需要 1-3 个工作日
- **更新版本**：通常需要 1-2 个工作日
- **节假日期间**：可能延长至 5-7 个工作日

### 5.2 审核标准
JetBrains 会检查以下方面：
- **功能性**：插件是否按描述工作
- **稳定性**：是否存在崩溃或严重 Bug
- **安全性**：是否包含恶意代码
- **合规性**：是否符合平台政策
- **质量**：代码质量和用户体验

### 5.3 常见审核问题
- **描述不准确**：功能描述与实际不符
- **兼容性问题**：在某些 IDE 版本中无法工作
- **权限过度**：请求不必要的系统权限
- **UI 问题**：界面不符合 JetBrains 设计规范

## ✅ 第六步：发布后管理

### 6.1 监控反馈
- 定期检查插件页面的用户评价
- 关注 GitHub Issues 中的问题报告
- 监控下载量和使用统计

### 6.2 版本更新流程
1. **修复 Bug 或添加新功能**
2. **更新版本号**：
   ```kotlin
   version = "1.0.1"  // 在 build.gradle.kts 中
   ```
3. **更新 change-notes**：
   ```kotlin
   changeNotes = """
       <h3>v1.0.1 - Bug 修复</h3>
       <ul>
           <li>🐛 修复文件路径处理问题</li>
           <li>💡 改进错误提示信息</li>
       </ul>
   """.trimIndent()
   ```
4. **构建并发布新版本**

### 6.3 用户支持
- 及时回复用户评论和问题
- 维护详细的文档和 FAQ
- 提供多种联系方式

## 📊 发布检查表

### 发布前最终检查
- [ ] 版本号已更新
- [ ] 更新日志已完善
- [ ] 所有功能测试通过
- [ ] 文档已更新
- [ ] 截图已准备
- [ ] 发布令牌已配置

### 发布后跟进
- [ ] 确认插件在市场中可见
- [ ] 测试从市场安装
- [ ] 监控初期用户反馈
- [ ] 准备快速修复计划

## 🚨 常见问题解决

### Q: 插件审核被拒绝怎么办？
A: 
1. 仔细阅读拒绝原因
2. 根据反馈修改代码
3. 更新版本号
4. 重新提交审核

### Q: 如何处理兼容性问题？
A:
1. 在多个 IDE 版本中测试
2. 使用条件编译处理 API 差异
3. 适当设置 `since-build` 和 `until-build`

### Q: 如何提高插件可见性？
A:
1. 选择准确的分类和标签
2. 提供清晰的描述和截图
3. 积极回应用户反馈
4. 在社区中推广

## 📚 参考资源

- [JetBrains 插件开发文档](https://plugins.jetbrains.com/docs/intellij/)
- [插件市场最佳实践](https://plugins.jetbrains.com/docs/marketplace/best-practices-for-listing.html)
- [IntelliJ Platform Gradle Plugin](https://plugins.jetbrains.com/docs/intellij/tools-intellij-platform-gradle-plugin.html)
- [插件发布指南](https://plugins.jetbrains.com/docs/intellij/publishing-plugin.html)

---

**祝你发布成功！** 🎉

如有问题，请参考官方文档或在 GitHub Issues 中提问。